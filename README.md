...
| LLM (OpenAI `gpt-5.5` or Anthropic `claude-opus-5`) | Extract, plan retrieval queries, propose events + reference class, word agent questions, narrate a finished log. Every call is a Pydantic schema. `llm.py` has no store, client, or credentials. `HEREAFTER_LLM=off`: sim, solidity, expiry, merge still run; events render as structured `text`. |
