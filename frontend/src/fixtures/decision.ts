...
export type Action = 'talk' | 'follow' | 'bury'
export const ACTIONS: Action[] = ['talk', 'follow', 'bury']
...
export const PAYOFF: Record<Action, { interested: number; not: number }> = {
  talk: { interested: 100, not: -15 },
  follow: { interested: -30, not: -60 },
  bury: { interested: 5, not: 5 },
}

export const LABEL: Record<Action, string> = { talk: 'Talk to her', follow: 'Follow her around', bury: 'Bury it and forget about it' }
...
export function analyse(steps: Evidence[], prior = PRIOR): Analysis {
  ...
  return { p, prior, steps, actions, recommended: best.action, threshold, responds: respondsWell(p), margin: p - threshold }
}
